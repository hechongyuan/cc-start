# -*- coding: utf-8 -*-
"""桌面番茄钟挂件 (Pomodoro desktop widget)
...
# ---------- 配色 (浅紫薰衣草主题) ----------
BG_CARD     = "#E8C6FF"   # 主卡片背景（用户指定的浅紫色）
BG_STRIP    = "#D6B0F5"   # 设置面板背景（比主卡片略深一点）
BTN         = "#C69BEA"   # 按钮底色
BTN_HOVER   = "#B588D8"   # 按钮悬停色
TEXT        = "#4A3070"   # 主文字（深紫色，保证可读）
TEXT_DIM    = "#7A5FA0"   # 次要文字（中紫色）
WORK_ACC    = "#8B2FC9"   # 专注模式强调色（深紫）
BREAK_ACC   = "#5E9A5A"   # 休息模式强调色（深绿）
TRACK       = "#D9B8F0"   # 进度环底轨
...
RING_R, RING_W   = 80, 12                             # 进度环的半径和线条宽度
BTN_ROW_Y = 254                                       # 一排控制按钮所在的 y 坐标
CLOSE_X, CLOSE_Y = 242, 26                            # 关闭按钮（✕）的圆心坐标
STRIP_TOP, STRIP_BOT = 286, 366                       # 设置面板的上边界、下边界 y 坐标
...
PHASES = {                                            # 阶段字典：key 为内部名，value 为该阶段的配置
    "work":  {"label": "专注中", "acc": WORK_ACC,     # work 阶段：显示名「专注中」、进度环用紫色
              "minutes_attr": "work_min",             #         时长存在哪个属性上
              "next": "break",                        #         结束后切换到 break
              "notify": ("工作结束", "干得漂亮！休息 {mins} 分钟吧")},  # 结束时的通知标题和内容
    "break": {"label": "休息中", "acc": BREAK_ACC,    # break 阶段：显示名「休息中」、进度环用绿色
...
class PomodoroWidget:                                 # 定义番茄钟挂件的主类
    ...
    def _btn(self, name, x, y, r, text, cmd):         # 在画布上创建一个圆形按钮
        ...
        tid = self.canvas.create_text(x, y, text=text, fill=TEXT,
                                      font=(FONT, int(r*0.95), "bold"))  # 在圆心画按钮文字
    ...
    def _draw(self):                                  # 重绘整个界面（每次状态变化时调用）
        ...
        # 主卡片
        self._rrect(6, 6, 254, 286, CARD_R, fill=BG_CARD)   # 画主卡片的圆角矩形背景

        # 进度环底轨
        self._ring_arc(359.9, TRACK)                  # 画几乎一整圈的圆弧作为进度环底轨

        # 文字
        self.time_text = c.create_text(RING_CX, 112, text=self._fmt(self.remaining),
                                       fill=TEXT, font=(FONT, 30, "bold"))  # 环中心显示倒计时时间
        self.mode_text = c.create_text(RING_CX, 154, text=self._mode_label(),
                                       fill=TEXT_DIM, font=(FONT_CN, 11))   # 环下方显示「专注中/休息中」
    ...
    def _make_spin(self, key, x, y):                  # 创建设置面板里的一行「标签 + 数字输入框」
        ...
        sb = tk.Spinbox(self.root, from_=1, to=120, width=7, justify="center",
                        bg=BG_CARD, fg=TEXT, buttonbackground=BTN,
    ...
    def _update_display(self):                        # 更新显示：进度环、时间、模式文字、播放按钮
    ...
    def _notify(self, title, msg):                    # 弹出到点通知小窗口
        ...
        win.configure(bg=BG_CARD)                     # 背景色与主卡片一致
        ...
        tk.Label(win, text=title, bg=BG_CARD, fg=WORK_ACC,
                 font=(FONT_CN, 15, "bold")).pack(pady=(16, 4))  # 标题文字
        tk.Label(win, text=msg, bg=BG_CARD, fg=TEXT,
                 font=(FONT_CN, 10)).pack()           # 正文文字
